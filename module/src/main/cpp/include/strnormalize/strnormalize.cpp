...
const unsigned short *_initTrad2Simp_utf16()
{
	...
	for (c = 0; c < n; c += 2)
	{
		...
		if (!((_pTns[c] >> 8) >= 0xA1 && (_pTns[c] >> 8) < 0xF8 && (_pTns[c] & 0xFF) >= 0xA1))
...
void str_normalize_utf8(char *text, unsigned int options)
{
	const char *pTransTable =(options & SNO_TO_LOWER) ? _pUpper2Lower : ((options & SNO_TO_UPPER) ? _pLower2Upper :_pPlain);
	const unsigned short *pTransTable_utf16 =(options & SNO_TO_SIMPLIFIED) ? _pTrad2Simp_utf16 : _pPlain_utf16;
	unsigned int i_from = 0, i_to = 0;
	while (text[i_from])
	{
		...
		else if ((text[i_from] & 0xF0) == 0xE0 && (text[i_from + 1] & 0xC0) == 0x80 && (text[i_from + 2] & 0xC0) == 0x80)
		{
			unsigned short utf16 = (unsigned short)(text[i_from] & 0x0F) << 12 | (unsigned short)(text[i_from + 1] & 0x3F) << 6 | (unsigned short)(text[i_from + 2] & 0x3F);
		...
		else if ((text[i_from] & 0xE0) == 0xC0 && (text[i_from + 1] & 0xC0) == 0x80)
		{
...
void str_normalize_gbk(char *text, unsigned int options)
{
	const char *pTransTable =(options & SNO_TO_LOWER) ? _pUpper2Lower : ((options & SNO_TO_UPPER) ? _pLower2Upper :_pPlain);
	const unsigned short *pTransTable_gbk =(options & SNO_TO_SIMPLIFIED) ? _pTrad2Simp_gbk : _pPlain_gbk;
	unsigned int i_from = 0, i_to = 0, flag = 0;
	for (; text[i_from]; i_from ++)
	{
		if (flag && (options & SNO_TO_HALF))
		{
			if ((unsigned char)text[i_from - 1] == 0xA1 && (unsigned char)text[i_from] == 0xA1)
			{
				flag = 0;
				text[i_from] = ' ';
			}
			else if ((unsigned char)text[i_from - 1] == 0xA3 && (char)text[i_from] < 0 && (unsigned char)text[i_from] > 0xA0)
			{
				flag = 0;
				text[i_from] &= ~0x80;
			}
		...
		{
			flag = 0;
			unsigned short tmp =pTransTable_gbk[COMPBYTE(text[i_from - 1], text[i_from]) & ~0x8000];
			text[i_to ++] = tmp & 0xFF;
			text[i_to ++] = tmp >> 8;
		}
...
{
	char *result =to;
	unsigned short i_from = 0;
	unsigned short i_to = 0;
	unsigned short flag = 0;
	if(from_len ==0 ||from ==NULL ||to ==NULL ||result ==NULL)	return -1;
	for(i_from =0; i_from <from_len; i_from++)
	{
		...
		else if((unsigned char)from[i_from] >=0x80)
		{
			flag = 1;
		}
		else
...
int utf8_to_gbk(const char *from, unsigned int from_len, char *to, unsigned int *to_len)
{
	...
	for(i_from =0; i_from <from_len; )
	{
		...
		else if((unsigned char)from[i_from] <0xE0)
		{
			...
			unsigned short tmp = _pUtf162Gbk[((from[i_from] & 0x1F) << 6) | (from[i_from + 1] & 0x3F)];
			if (tmp)
			{
				result[i_to ++] =(tmp >> 8);
		...
		else if((unsigned char)from[i_from] <0xF0)
		{
			...
			unsigned short tmp = _pUtf162Gbk[((from[i_from] & 0x0F) << 12)| ((from[i_from + 1] & 0x3F) << 6) | (from[i_from + 2] & 0x3F)];
			if(tmp)
			{
				result[i_to ++] =(tmp >> 8);
				result[i_to ++] =(tmp & 0xFF);
